...
export class CodeSyncResourceDB {
        ...
        const resources = await connection.manager
            ...
            .where(qb => {
                const subQuery = qb.subQuery()
                    ...
                    .where('resource2.userId = :userId', { userId })
    ...
    async getResources(userId: string, kind: ServerResource): Promise<DBCodeSyncResource[]> {
        const connection = await this.typeORM.getConnection();
        return connection.manager.getRepository(DBCodeSyncResource)
            .createQueryBuilder('resource')
            .where('resource.userId = :userId AND resource.kind = :kind', { userId, kind })
